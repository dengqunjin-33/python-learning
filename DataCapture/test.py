...
# 爬取接口地址
url = "https://flash-api.jin10.com/get_flash_list1"
# 爬取数据页面地址(备用)
...
def saveJinShiInfo(id, dataJson, my_cursor):
    ...
    for index in range(0, len(keyList)):
        if keyList[index] in dataJson:
            dataList.append(dataJson[keyList[index]].replace('<b>', '').replace('</b>', ''))
        else:
            dataList.append(None)
...
def doJob():
    ...
    # 添加任务,时间间隔5S
    scheduler.add_job(doSave, 'interval', max_instances=100, seconds=5)
    scheduler.start()
